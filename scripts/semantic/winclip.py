...
if __name__ == "__main__":
    ...
    for i, n_sample in enumerate([0, 1, 2, 4, 8, 16, 32, 64]):
        if n_sample > 0:
            attend_metrics_list = []
            ignore_metrics_list = []
            ...
            metrics[attend_name][f"Image + Language/WinCLIP+/{n_sample}"] = attend_metrics_list
            metrics[ignore_name][f"Image + Language/WinCLIP+/{n_sample}"] = ignore_metrics_list
            ...
            metrics[attend_name][f"Language/WinCLIP/{n_sample}"] = laft.binary_metrics(scores, attend_labels)
            metrics[ignore_name][f"Language/WinCLIP/{n_sample}"] = laft.binary_metrics(scores, ignore_labels)
